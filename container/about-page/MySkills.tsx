...
export default function MySkills() {
	const [activeAccordion, setActiveAccordion] = useState(skillsItems[0].id);
	...
	return (
		<section className="w-full padding-y">
			<div className="w-full padding-x mb-[40px]">
				<h1 className="sub-heading font-medium font-NeueMontreal text-secondry">My Skills</h1>
			</div>
				...
				<div key={item.id} className={`w-full flex py-[10px] flex-col ${item.id == 1 ? 'border-y border-[#21212155]' : 'border-b border-[#21212155]'}`}>
					<div className="w-full flex items-center justify-between py-[10px] padding-x">
						<div className="w-[50%] sm:w-full xm:w-full">
							<h3 className="paragraph font-normal font-NeueMontreal text-secondry">{item.phase}</h3>
						</div>
						<div className="w-[40%] sm:w-full xm:w-full">
							<h3 className="paragraph font-normal font-NeueMontreal text-secondry">{item.name}</h3>
						</div>
